...
- **Browse the Dashboard for earlier days** — the Dashboard gains a date selector with day-back/forward arrows so you can review how the system actually behaved on a past day (energy flows, schedule, SOC), the way the Growatt app lets you page back. Historical days are read from the persisted daily-view store; live-only widgets (System Overview, the "now" marker, tomorrow's plan) are hidden when viewing the past.
